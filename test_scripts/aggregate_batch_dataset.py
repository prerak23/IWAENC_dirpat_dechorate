...
path_noisy_mix="/srv/storage/talc3@talc-data.nancy/multispeech/calcul/users/psrivast/IWAENC/test_400_rooms/D5/"
new_file=h5py.File(path_noisy_mix+"D5_1111_aggregated_mixture.hdf5","w")
room_save=new_file.create_group("room_nos")

parallel_jobs=100

for file_name in os.listdir(path_noisy_mix):
    if "noisy_mixture_rooms_" in file_name:
...
#-----------Change this path to aggregate different batch datasets -------------------------------
path_noisy_mix="/srv/storage/talc3@talc-data.nancy/multispeech/calcul/users/psrivast/IWAENC/test_400_rooms/D5/"
new_file_anno=h5py.File(path_noisy_mix+"D5_1111_aggregated_mixture_annotations.hdf5","w")
room_save=new_file_anno.create_group("room_nos")
parallel_jobs=100
vol_arr=np.empty((1,400))
surf_area=np.empty((1,400))
rt60=np.empty((6,400))
