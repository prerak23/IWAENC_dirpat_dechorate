...
chkp=torch.load("/srv/storage/talc3@talc-data.nancy/multispeech/calcul/users/psrivast/IWAENC/training/D5_1111/mlh_tas_save_best_sh.pt",map_location=device)
...
no_of_rooms=np.arange(36000,36400)
...
estimate=np.zeros((1,16))


for r in no_of_rooms:

    bnf_mixture_ch1,bnf_mixture_ch2,rt60,surf,vol=test_data.return_data(r)
    ...
    total_mean_local=np.zeros((1,8))

    total_variance_local=np.zeros((1,8))

    total_precision=np.zeros((1,8))

    total_num=np.zeros((1,8))

    f=np.concatenate((surf.reshape(1,1),vol.reshape(1,1),rt60.reshape(1,6),),axis=1)
    ...
    for vp in range(no_of_vps):
        ...
        x=x.reshape(1,-1)
        '''
        mean,variance=model_4(feat_1,feat_2,0)
...
np.save("/home/psrivastava/axis-2/IWAENC/z_test/D5_test_set/vp_3_test_same_src.npy",estimate)
