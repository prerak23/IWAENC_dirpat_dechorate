...
class conf_files(util_room,util_receiver):
    ...
    def room_file(self):
        ...
        for x in tqdm(range(self.number_rooms)):
            ...
            dict_file_source['room_' + str(x)]={'source_pos':li_sc.tolist(),'source_ypr':li_sc_ypr.tolist(),'directivity':random.choice([0,4,5])}
        ...
        with open('conf_room_setup_D7_1101.yml', 'w') as file:
            documents = yaml.dump(dict_file, file)
        with open('conf_receivers_D7_1101.yml','w') as file_1:
            documents = yaml.dump(dict_file_receiver,file_1)
        with open('conf_source_D7_1101.yml','w') as file_2:
            documents = yaml.dump(dict_file_source,file_2)
        with open('conf_noise_source_D7_1101.yml','w') as file_3:
