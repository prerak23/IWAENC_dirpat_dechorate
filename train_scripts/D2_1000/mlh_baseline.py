...
def cal_features(ch1, ch2):
    ...
    enc_ch2 = torch.stft(ch2.view(16, -1), n_fft=768, hop_length=384, return_complex=True)
...
std_volume = 72.760007
std_surface = 64.187581

std_rt60=[ 0.395032, 0.376963, 0.297600, 0.263775, 0.242831, 0.214957] #For 125,250,500,1000,2000,4000 Hz
...
#vari_surface=4157.618919653288
#vari_rt60=[0.06384,0.066428,0.04562,0.02882,0.02324,0.02023]
...
def train(model, train_loader, optimizer, epoch, ar_loss, batch_loss_ar):
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        surface = surface.reshape(16, 1)

        volume = volume.reshape(16, 1)
        ...
        loss_surface = NLLloss(surface.view(16) / std_surface, mean[:, 0], variance[:, 0])

        loss_volume = NLLloss(volume.view(16) / std_volume, mean[:, 1], variance[:, 1])
        ...
        surface_mse += float(pred(mean[:, 0] * std_surface, surface.view(16)).item())

        volume_mse += float(pred(mean[:, 1] * std_volume, volume.view(16)).item())
        ...
        track_var = np.concatenate((track_var, variance.detach().cpu().clone().numpy().reshape(16, 8)), axis=0)


        if epoch == 0:
            adcc = np.concatenate((adcc, total.detach().cpu().clone().numpy().reshape(16, 8)), axis=0)
...
def val(model, train_loader, optimizer, epoch, val_data_ar, acc_data_ar):
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        surface_loss_mse_t += pred(mean[:, 0] * std_surface, surface.view(16))
        volume_loss_mse_t += pred(mean[:, 1] * std_volume, volume.view(16))
        ...
        target_ = torch.cat((surface, volume, rt60,  rm.reshape(16, 1), vp.reshape(16, 1)), axis=1)  # absorption , rt60 (real annotations)
        ...
        analysis_data = np.concatenate((output.detach().cpu().clone().numpy().reshape(16, 16),
                              target_.detach().cpu().clone().numpy().reshape(16, 10)), axis=1)
...
#a1,b1=cal_features(torch.randn(16,1,48000),torch.randn(16,1,48000))
#print(a1.shape)
#net(a1,b1,1)
...
path="/srv/storage/talc3@talc-data.nancy/multispeech/calcul/users/psrivast/IWAENC/training/D2_1000/"
