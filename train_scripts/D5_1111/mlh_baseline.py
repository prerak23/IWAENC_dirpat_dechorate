...
def cal_features(ch1, ch2):
    enc_ch1 = torch.stft(ch1.view(32, -1), n_fft=768, hop_length=384, return_complex=True)

    enc_ch2 = torch.stft(ch2.view(32, -1), n_fft=768, hop_length=384, return_complex=True)
    ...
    #const=torch.ones(16,769,63)*1e-7
...
std_rt60=[ 0.40875, 0.38669, 0.303711, 0.268418, 0.273678, 0.239206] #For 125,250,500,1000,2000,4000 Hz
...
def train(model, train_loader, optimizer, epoch, ar_loss, batch_loss_ar):
    ...
    adcc = np.zeros((1, 8))  # Total 8 Values are saved for the purpose of analysis of the data after the model is trained.

    track_var = np.zeros((1, 8))  # Total 8 values for the tracking of the variance while the model is getting trained
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        surface = surface.reshape(32, 1)

        volume = volume.reshape(32, 1)
        ...
        loss_surface = NLLloss(surface.view(32) / std_surface, mean[:, 0], variance[:, 0])

        loss_volume = NLLloss(volume.view(32) / std_volume, mean[:, 1], variance[:, 1])
        ...
        loss = (rt60_loss +  loss_surface + loss_volume) / 8
        ...
        surface_mse += float(pred(mean[:, 0] * std_surface, surface.view(32)).item())

        volume_mse += float(pred(mean[:, 1] * std_volume, volume.view(32)).item())
        ...
        total = torch.cat((surface, volume, rt60), axis=1)

        # For tracking the variance while traning.

        track_var = np.concatenate((track_var, variance.detach().cpu().clone().numpy().reshape(32, 8)), axis=0)


        if epoch == 0:

            adcc = np.concatenate((adcc, total.detach().cpu().clone().numpy().reshape(32, 8)), axis=0)



        del loss, data, loss_surface, loss_volume, rt60_loss

        #Save MSE loss after 100 epochs.

        if batch_idx % 50 == 49:
            # print("Running loss after 100 batches",(loss_batch/100),loss_batch)
            batch_loss_ar.append((surface_mse / 50, volume_mse / 50, (rt60_125_) / 50, (rt60_250_) / 50,
                                  (rt60_500_) / 50, (rt60_1000_) / 50, (rt60_2000_) / 50, (rt60_4000_) / 50))
...
def val(model, train_loader, optimizer, epoch, val_data_ar, acc_data_ar):
    ...
    local_data_sp = np.zeros([1, 26])  # Save validation data(real annotations params + estimated params 8(mean)+8(varaince)+8(real annotations)+room_id(1)+vp_id(1) ~ 26 )  for analysis purposes 42
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        val_loss_t = (loss_surface + loss_volume + rt60_loss ) / 8
        ...
        surface_loss_mse_t += pred(mean[:, 0] * std_surface, surface.view(32))
        volume_loss_mse_t += pred(mean[:, 1] * std_volume, volume.view(32))
        ...
        target_ = torch.cat((surface, volume, rt60,  rm.reshape(32, 1), vp.reshape(32, 1)), axis=1)  # absorption , rt60 (real annotations)
        ...
        analysis_data = np.concatenate((output.detach().cpu().clone().numpy().reshape(32, 16),
                              target_.detach().cpu().clone().numpy().reshape(32, 10)), axis=1)
        ...
        del val_loss_t, volume_loss_mse_t, surface_loss_mse_t, rt60_loss_mse_125_t, rt60_loss_mse_250_t, rt60_loss_mse_500_t, rt60_loss_mse_1000_t, rt60_loss_mse_2000_t, rt60_loss_mse_4000_t
        # del val_loss_t,volume_loss_mse_t,surface_loss_mse_t

        #Track mse after every 50 batches.

        if batch_idx % 25 == 24:
            surface_acc = (surface_loss_mse / 25)

            volume_acc = (volume_loss_mse / 25)

            rt60_125_acc = (rt60_loss_mse_125 / 25)

            rt60_250_acc = (rt60_loss_mse_250 / 25)
            ...
            del surface_acc, volume_acc, rt60_125_acc, rt60_250_acc, rt60_500_acc, rt60_1000_acc, rt60_2000_acc, rt60_4000_acc
...
class Model_1(torch.nn.Module):
    def __init__(self):
        ...
        #Pipeline 2
        self.pip_conv_1d = nn.Conv1d(1155, 1155, kernel_size=11, stride=1, groups=1155, padding=5) # ks=2,st=2
        self.pip_conv_1p = nn.Conv1d(1155, 576, kernel_size=1, stride=1, padding=0)
        self.ln_pip_1 = nn.LayerNorm([576])
    ...
    def forward(self, x, x2):
        ...
        x2=self.ln_pip_1(x2.reshape(self.batch_size,-1,576))

        x2=self.drp_1(x2.reshape(self.batch_size,576,-1))
        #print(x2.shape)
...
class Model_3(torch.nn.Module):
    def __init__(self):
        ...
        self.batch_size = 32
        self.drp = nn.Dropout(p=0.5)
        self.fc_1 = nn.Linear(624, 96)
        self.fc_2 = nn.Linear(96, 48)
        self.fc_3 = nn.Linear(48, 16)
...
train_dl = DataLoader(train_data, batch_size=32, shuffle=True, num_workers=0, drop_last=True)
val_dl = DataLoader(val_data, batch_size=32, shuffle=True, num_workers=0, drop_last=True)
...
net = Ensemble(net_1, net_3,32).to(torch.device("cuda"))
...
save_best_val = 0
adcc = np.zeros((1, 8))
track_var = np.zeros((1, 8))
local_dt_sp = np.zeros((1, 26))
...
path="/srv/storage/talc3@talc-data.nancy/multispeech/calcul/users/psrivast/IWAENC/training/D5_1111/mse_training/"
