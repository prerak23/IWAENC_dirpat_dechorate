...
def train(model, train_loader, optimizer, epoch, ar_loss, batch_loss_ar):
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        track_var = np.concatenate((track_var, variance.detach().cpu().clone().numpy().reshape(128, 8)), axis=0)


        if epoch == 0:
            adcc = np.concatenate((adcc, total.detach().cpu().clone().numpy().reshape(128, 8)), axis=0)
...
def val(model, train_loader, optimizer, epoch, val_data_ar, acc_data_ar):
    ...
    for batch_idx, sample_batched in enumerate(train_loader):
        ...
        target_ = torch.cat((surface, volume, rt60,  rm.reshape(128, 1), vp.reshape(128, 1)), axis=1)  # absorption , rt60 (real annotations)
        ...
        analysis_data = np.concatenate((output.detach().cpu().clone().numpy().reshape(128, 16),
                              target_.detach().cpu().clone().numpy().reshape(128, 10)), axis=1)
...
class Model_1(torch.nn.Module):
    def __init__(self):
        ...
        '''
        self.pip_conv_2d = nn.Conv1d(1152, 1152, kernel_size=11, stride=1, groups=1152, dilation=2, padding=10)
        self.pip_conv_2p = nn.Conv1d(1152, 576, kernel_size=1, stride=1, padding=0)
        self.ln_pip_2 = nn.LayerNorm([576])
        '''
        ...
        '''
        self.pip_conv_3d = nn.Conv1d(576, 576, kernel_size=11, stride=1, groups=576, dilation=4, padding=20)
        self.pip_conv_3p = nn.Conv1d(576, 288, kernel_size=1, stride=1)
        self.ln_pip_3 = nn.LayerNorm([288])
        '''
    ...
    def forward(self, x, x2):
        ...
        #print(x2)
        x2=self.relu(self.pip_conv_2p(x2))
        x2=self.ln_pip_2(x2.reshape(self.batch_size,-1,576))
        ...
        #print(x2)
        x2=self.relu(self.pip_conv_3p(x2))

        x2=self.ln_pip_3(x2.reshape(self.batch_size,-1,288))
        x2=self.drp_1(x2.reshape(self.batch_size,288,-1))
        '''
...
train_data = dl.binuaral_dataset('/home/psrivastava/baseline/scripts/pre_processing/train_random_ar.npy')
val_data = dl.binuaral_dataset('/home/psrivastava/baseline/scripts/pre_processing/val_random_ar.npy')
